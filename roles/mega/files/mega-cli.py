...
formatter = logging.Formatter('%(asctime)s %(name)s %(levelname)-6s %(message)s', '%Y-%m-%d %H:%M:%S')
handler.setFormatter(formatter)
logger.addHandler(handler)
logger.setLevel(logging.INFO)


MEGA_SYNC_COLUMNS = ["LOCALPATH", "REMOTEPATH", "RUN_STATE", "STATUS", "ERROR"]
...
def items_matched(item_to_configure, configured_item):
    return configured_item[0] == item_to_configure['local'] and configured_item[1] == item_to_configure['remote']
